...
def print_colored(message: str, color: str = ""):
    ...
    colors = {'red': '\033[0;31m', 'green': '\033[0;32m', 'blue': '\033[0;34m', 'yellow': '\033[1;33m', 'reset': '\033[0m'}
...
def open_port(args):
    ...
    if not all([args.name, args.local_address, args.local_port, args.external_port]):
        if args.json:
            print(json.dumps({"status": "error", "message": "Missing required parameters for opening port. Required: --name, --local-address, --local-port, --external-port"}, indent=2))
        else:
    ...
    if not validate_port(args.local_port) or not validate_port(args.external_port):
        if args.json:
            print(json.dumps({"status": "error", "message": "Invalid port number(s)"}, indent=2))
        else:
    ...
    if not authenticated:
        if args.json:
            print(json.dumps({"status": "error", "message": "Failed to authenticate with router"}, indent=2))
        else:
    ...
    rule = PortForwardingRule(
            name=args.name, local_address=local_addr, local_port=int(args.local_port), external_port=int(args.external_port), protocol=args.protocol
    )
    ...
    if success:
        if args.json:
            rule_dict = {
                "name": rule.name, "local_address": rule.local_address, "local_port": rule.local_port,
                "external_port": rule.external_port, "protocol": rule.protocol
            }
            print(json.dumps({"status": "success", "message": "Port forward created successfully", "rule": rule_dict}, indent=2))
        else:
        ...
        if args.json:
            print(json.dumps({"status": "error", "message": "Failed to create port forward"}, indent=2))
        else:
...
def close_port(args):
    ...
    if not args.port:
        if args.json:
            print(json.dumps({"status": "error", "message": "External port number is required"}, indent=2))
        else:
    ...
    if not validate_port(args.port):
        if args.json:
            print(json.dumps({"status": "error", "message": "Invalid port number"}, indent=2))
        else:
    ...
    with Spinner(f"Removing port forward for external port: {args.port}...", silent=args.json):
        success = client.remove_port_forward_by_port(int(args.port))

    if success:
        if args.json:
            print(json.dumps({"status": "success", "message": f"Port forward for port {args.port} removed successfully"}, indent=2))
        else:
            log_success(f"Port forward for port {args.port} removed successfully")
        return True
    else:
        if args.json:
            print(json.dumps({"status": "error", "message": "Failed to remove port forward"}, indent=2))
        else:
...
def open_browser(args):
    ...
    if args.json:
        print(json.dumps({"status": "success", "message": "Session URL retrieved.", "url": session_url}, indent=2))
        return True

    log_info("Opening router web interface...")

    try:
        subprocess.run(["open", session_url], check=True, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
...
complete -F _sagemcom_cli_completion sagemcom-cli
        ...
        print("# Note: You may need to restart your shell for changes to take effect.", file=sys.stderr)
        return True
    return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
            description="Sagemcom Router Port Forward Manager", formatter_class=argparse.RawDescriptionHelpFormatter, epilog="""
Examples:
  ...
  sagemcom-cli completion bash
    ...
    parser.add_argument(
        '--host', default=default_host, help=f'Router IP address (default: {default_host})'
    )
    parser.add_argument(
    ...
    open_parser.add_argument('--name', required=True, help='Name for the port forwarding rule')
    open_parser.add_argument(
        '--local-address', required=True, help='Local IP address (full IP or shorthand like "100")'
    )
    open_parser.add_argument('--local-port', required=True, type=int, help='Local port number')
    open_parser.add_argument('--external-port', required=True, type=int, help='External port number')
    open_parser.add_argument(
        '--protocol', choices=['tcp', 'udp', 'tcp_udp'], default='tcp', help='Protocol (default: tcp)'
    )

    # Close port command
    close_parser = subparsers.add_parser('close', help='Close a port forwarding rule')
    close_parser.add_argument('--port', required=True, type=int, help='External port number of the rule to remove')

    # List ports command
    list_parser = subparsers.add_parser('list', help='List all port forwarding rules')

    # Browser command
    browser_parser = subparsers.add_parser('browser', help='Open router web interface in browser')

    # Completion command
    completion_parser = subparsers.add_parser('completion', help='Generate shell completion script')
    completion_parser.add_argument('shell', choices=['bash'], default='bash', nargs='?', help='The shell to generate completion for (default: bash)')
